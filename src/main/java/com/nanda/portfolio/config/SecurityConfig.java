...
public class SecurityConfig {
 ...
 @Bean SecurityFilterChain security(HttpSecurity http, AdminProperties p) throws Exception {
  return http.authorizeHttpRequests(a->a.requestMatchers("/admin/login","/css/**","/js/**","/images/**","/portfolio/**","/api/csrf","/api/contact","/api/portfolio/**","/resume.pdf","/","/contact","/error","/robots.txt","/sitemap.xml").permitAll().requestMatchers("/admin/**","/api/admin/**").hasRole("ADMIN").anyRequest().permitAll())
